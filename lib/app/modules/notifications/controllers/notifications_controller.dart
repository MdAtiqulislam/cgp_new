...
class NotificationsController extends GetxController {
  ...
  void handelClick({required SingleNotificationModel notification}) {
    ...
    if ( notification.data?.orderId!=null) {
      ...
      Get.find<OrderDetailsController>().getOrderDetails(orderId: notification.data?.orderId??"");
      Get.toNamed(Routes.ORDER_DETAILS);

      /*if(notification.data?.type=="delivery"){
  ...
  void markAsRead({required String notificationId})async {
    var endPoint=APIEndPoints.markNotificationAsRead.replaceAll("{notificationId}", notificationId);
